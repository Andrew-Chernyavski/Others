...
{
    MainWindow::ui->textEditOpenedMap->append("  Stations of line " + QString::number(line) + " haham:");
    QMap<string, Station*>::iterator ind;
    for (ind = openedMap->stations.begin(); ind != openedMap->stations.end(); ind++)
        if (ind.value()->line == line)
        {
            MainWindow::ui->textEditOpenedMap->append("  *  " + QString::fromStdString(ind.value()->nameOfStation + "; x: " + QString::number(ind.value()->x + "; y: " + QString::number(ind.value()->y)));
            ...
            while (!xmlReader.atEnd() && !xmlReader.hasError())
            {
                ...
                if (xmlReader.isStartElement() && xmlReader.name() == "Map")
                {
                    ...
                    while (xmlReader.name() == "Station")
                    {
                        ...
                        readNextNTimes(xmlReader, 4);
                        newStation->x = xmlReader.text().toInt();
                        readNextNTimes(xmlReader, 4);
                        newStation->y = xmlReader.text().toInt();
